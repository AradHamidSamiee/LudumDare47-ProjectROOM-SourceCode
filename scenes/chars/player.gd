...
onready var env_set = preload("res://set_env.tres")
onready var env_def = preload("res://default_env.tres")
...
func crt_shader():
	...
	if crtison == "True":
		$crt.show()
		$head/Camera.environment = env_set
	else:
		$crt.hide()
		$head/Camera.environment = env_def
...
func _on_jumpscare_handler_body_entered(body):
